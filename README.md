...
`fast` 是无外部 EDA 进程的 hermetic 门禁；`regression`、`nightly`、fixture prepare/validation 涉及 NPI、MCP 进程或 VCS 时必须在沙箱外执行。cache miss 对 required suite 是 ERROR，并给出精确 prepare 命令；不会自动 prepare、SKIP 或切换 backend。裸 `pytest` 是 usage error。完整合同见 [`doc/agents/xdebug/tests.md`](doc/agents/xdebug/tests.md)。

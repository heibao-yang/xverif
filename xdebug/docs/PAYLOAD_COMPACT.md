...
细节通过 action-specific schema 允许的 `include_*` 开关恢复；不要把下面示例当成所有 action 都支持的公共字段：
...
compact 默认返回 summary、count 或按 `query.index`/`query.limit` 返回的 transaction 数据。不要使用旧 `include_transactions`；需要完整导出时使用对应 export action。

### apb.query

compact 默认返回 summary、count 或按 `query.index`/`query.limit` 返回的 access 数据。不要使用旧 `include_accesses`；需要窗口分析时使用 `apb.transfer_window`。

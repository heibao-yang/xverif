# xdebug AXI export implementation plan
...
默认 xout 只显示路径、count、窗口和排序语义，不内嵌 transaction rows。
...
- `xdebug/README.md` 和 help text。
- compact xout 渲染。
- MCP smoke action list 或泛型 query smoke。

MCP 不新增独立 tool。AI 和 MCP client 通过已有
`xverif_debug_session_open` + `xverif_debug_query(action="axi.export")`
进入。
...
- TSV 和 CSV smoke。
- `xverif_debug_query(action="axi.export")` MCP smoke。

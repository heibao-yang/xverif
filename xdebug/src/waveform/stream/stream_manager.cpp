...
bool StreamManager::load_session(const std::string& session_id, std::vector<StreamConfig>& configs) {
    configs.clear();
    int fd = open(xdebug_waveform_streams_path(session_id).c_str(), O_RDONLY);
    if (fd < 0) return true;
    if (!lock_file(fd)) {
        close(fd);
        return false;
...
bool StreamManager::save_session(const std::string& session_id, const std::vector<StreamConfig>& configs) {
    ...
    int fd = open(xdebug_waveform_streams_path(session_id).c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0600);

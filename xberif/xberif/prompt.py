...
RULES = """You are initializing xberif.

Rules:
1. Create or update summary cards by writing JSON files directly under .xberif/cards/.
2. Create topic details by writing Markdown files directly under .xberif/details/.
...
5. Evidence path must exist in .xberif/manifest.json.
6. If unsure, use unknown or confidence: low.
7. Only create cards for topics defined in xberif/topics.toml.
8. Do not create topics from another env_kind.
9. Do not output prose as final result.
10. Do not run validation yourself; xberif hooks will validate card/detail files after Write/Edit and before stop.
...
def render_prompt(root: Path, kind_cfg: dict, topics_cfg: dict, manifest: dict) -> str:
    ...
    chunks = [
        ...
        f"- Write one JSON file per topic to .xberif/cards/{kind}.<topic>.json\n"
        f"- Write one Markdown file per topic to .xberif/details/{kind}.<topic>.md\n"
        "- Both filenames must exactly match the card id from xberif/topics.toml.\n"
        "- Do not create temporary generated files under /tmp.\n"
        "\n"
        "Allowed write/edit scope:\n"
        "- .xberif/cards/*.json\n"
        "- .xberif/details/*.md\n"
        "- Never edit xberif/*.toml, RTL, DV, docs, manifest.json, cards.json, kind.json, or lock.json.\n",
        "\nTopic card envelope schema: xberif.topic_card.v1\n"
        ...
        "\nExample .xberif/cards file content:\n"
        "{\n"
        ...
        f"  \"detail\": {{\"available\": true, \"path\": \".xberif/details/{kind}.project.md\", "
        "\"format\": \"markdown\", \"token_estimate\": 0, \"section_count\": 0}},\n"
        "  \"notes\": [],\n"
        "  \"unknowns\": [],\n"
        "  \"generated_by\": {\"tool\": \"claudecode\", \"xberif_version\": \"0.3.0\"}\n"
        "}\n"
        "\n",
        "\nTopic detail schema: xberif.topic_detail.v1\n"
    ...
    chunks.append(
        ...
        "4. Exit after all required card/detail pairs are written. xberif will rebuild cards.json and validate.\n"
    )
    return "\n".join(chunks)

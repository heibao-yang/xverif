...
本文是 `xdebug.v1` JSON API 的响应字段手册，覆盖当前 action catalog 中的所有命令。MCP 的 `xverif_debug_query` 在 `output_format:"json"` 时返回这些 backend JSON 字段；`output_format:"xout"` 返回适合 AI 阅读的文本摘要，`output_format:"envelope"` 用于定位 wrapper/stdio-loop 问题。它面向 AI agent 和脚本作者，目标是说明每个 action 成功响应里可能出现的 `summary`、`data`、`findings`、`meta`、`error` 字段，以及 compact/verbose 下字段是否会被省略。

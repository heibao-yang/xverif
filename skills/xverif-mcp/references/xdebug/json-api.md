...
MCP 返回格式只用 tool 顶层 `output_format` 控制，默认优先使用 `xout`。需要脚本读取字段时使用 `output_format:"json"`。不要在 native envelope 顶层写 `output`，也不要写 `args.output.format`；action 导出文件格式只放在 `args.output.file_format`。

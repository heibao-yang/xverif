...
默认优先使用 xout 输出；示例不写 `output_format`。只有脚本需要稳定读取 JSON 字段，或专门验证 JSON response/schema 时，才显式请求 JSON。MCP 不暴露完整 `xcov.v1` 原生 envelope；需要原生 request 时使用 `xverif-cli`。

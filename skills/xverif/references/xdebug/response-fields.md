...
| --- | --- | --- |
| `summary.name` | string | AXI config 名 |
| `summary.analysis` | string | `latency`、`osd` 或 `outstanding` |
...
| `data.outstanding[]` | array | `analysis=outstanding` 的 pending transaction |
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `addr_time` / `first_data_time` / `last_data_time` / `resp_time` | string | phase handshake 时间 |
| `phase_order` | string | `aw_before_w`、`same_cycle` 或 `w_before_aw` |
| `response_dependency_violation` | boolean | B 是否违反 AW/WLAST 依赖 |
| `match_time` | string | range 匹配时间 |
| `latency` | string | response - address |

...
SIGNAL_KEYS = ("rst_n", "psel", "penable", "pwrite", "paddr", "pwdata", "prdata", "pready", "pslverr")


def main() -> int:
    ap = argparse.ArgumentParser(description="Extract an APB transaction report from FSDB.")
    ap.add_argument("--fsdb", required=True)
    ap.add_argument("--config", required=True, help="JSON with clk and APB signal paths including pready/pslverr")
        ...
        try:
            ...
            with pynpi_lifecycle([sys.argv[0]]):
                ...
                try:
                    ...
                    result = apb_summary(scan, cfg, detail=args.detail)
                    result["meta"].update({"time_base": "fsdb_tick", "scale_unit": fp.scale_unit()})
                finally:
                    close_fsdb(fp)
            emit_result("apb_summary", result, args.detail, args.output, json_stream)
            return 0
        except Exception as exc:
            scan_meta = scan.context.as_dict() if scan is not None else None
            print_json(error_document("apb_summary", exc, scan_meta=scan_meta), json_stream)
            return 1
